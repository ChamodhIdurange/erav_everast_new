...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['vat_percentage'])) {
    ...
    if ($check->num_rows > 0) {
        $conn->query("UPDATE tbl_tax SET rate = '$vat', updated_at = NOW() WHERE id = (SELECT id FROM tbl_tax LIMIT 1)");
        echo json_encode(['status' => 'success', 'message' => 'VAT percentage updated successfully!']);
    } else {
        $conn->query("INSERT INTO tbl_tax (rate, created_at) VALUES ('$vat', NOW())");
